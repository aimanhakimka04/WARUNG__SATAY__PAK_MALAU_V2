...
if (isset($_POST["recover-submit"])) {
    $email = $_POST["email"];
    $d = $db->query("SELECT * FROM user_info WHERE email = '$email'");
    ...
    if ($d->num_rows > 0) {
        ...
        try {
            ...
            $message = file_get_contents("forgotpasswordtemplate.html");

            // Replace placeholders in the email template
            $message = str_replace("{{EMAIL}}", $email, $message);
            $resetUrl = "http://localhost/resetpassword.php?token=" . $url;
