...
<script>
  ...
  $('#qty-minus').click(function(){
    ...
    if(qty == 1){
      return false; //prevent negative qty
    }else{
      $('input[name="qty"]').val(parseInt(qty) -1); //this will decrement the qty
    }
  })
  ...
  $('#add_to_cart_modal').click(function(){
    start_load() //this is a function that will show the loading icon
    $.ajax({
      ...
      success:function(resp){
        if(resp == 1 )
          alert_toast("Order successfully added to cart"); //this is a function that will show the alert
